#!/usr/bin/env python3
"""
Herbies Seeds Comprehensive Scraper
Using 4-method extraction approach from COMPREHENSIVE_SCRAPING_STRATEGY.md
Target: 2000+ strains from feminized-seeds and autoflower-seeds collections
"""
...
class HerbiesStrainScraper:
    def __init__(self):
        self.base_url = "https://herbiesheadshop.com"
    ...
    def extract_strain_urls(self, collection_url):
        ...
        for selector in selectors:
            ...
            for link in links:
                href = link.get('href')
                if href and '/cannabis-seeds/' in href:
    ...
    def _extract_from_tables(self, soup, strain_data):
        ...
        tables = soup.find_all(['table', 'dl', 'ul'])
        
        for table in tables:
            if table.find_all(['dt', 'th', 'li']):
                rows = table.find_all(['tr', 'dt', 'li'])
                for row in rows:
    ...
    def _extract_from_descriptions(self, soup, strain_data):
        ...
        descriptions = soup.find_all(['div', 'p'], class_=re.compile(r'description|content|details'))
    ...
    def _extract_basic_info(self, soup, strain_data, url):
        ...
        for selector in title_selectors:
            ...
            if element:
                ...
                # Clean up title
                strain_name = re.sub(r'\s*-\s*Herbies Seeds.*', '', title, flags=re.IGNORECASE)
        ...
        if not strain_data.get('strain_name'):
            url_parts = url.split('/')
            if len(url_parts) > 0:
                strain_slug = url_parts[-1]
                strain_name = strain_slug.replace('-', ' ').title()
                strain_name = re.sub(r'\s*Herbies Seeds.*', '', strain_name, flags=re.IGNORECASE)
                strain_data['strain_name'] = strain_name.strip()
        ...
        if not strain_data.get('breeder_name'):
            strain_data['breeder_name'] = "Herbies Seeds"
    ...
    def save_to_dynamodb(self, strain_data):
        ...
        try:
            ...
            # Add metadata
            strain_data['source'] = 'herbies_seeds'
    ...
    def scrape_all_strains(self):
        ...
        print("Starting Herbies Seeds comprehensive scraping...")
        ...
        print(f"\nHERBIES SEEDS SCRAPING COMPLETE!")
