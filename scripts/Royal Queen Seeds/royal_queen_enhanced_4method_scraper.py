#!/usr/bin/env python3
"""
Royal Queen Seeds - Enhanced 4-Method Scraper
Target: 100-200 strains with 95%+ success rate
Hardcoded: seed_bank and breeder_name as 'Royal Queen Seeds'
"""
...
class RoyalQueenEnhanced4MethodScraper:
    def __init__(self):
    ...
    def method1_structured_extraction(self, soup, url):
        """Method 1: Royal Queen Seeds structured table extraction"""
        data = {}
        
        # Target the product features table
        table = soup.find('table', id='idTab2')
        if not table:
            table = soup.find('table', class_='product-features-list')
        
        if table:
            rows = table.find_all('tr')
            for row in rows:
                name_cell = row.find('th', class_='feature-name')
                value_cell = row.find('td', class_='feture-value')  # Note: typo in class name
                
                if name_cell and value_cell:
                    feature_name = name_cell.get_text().strip().rstrip(':')
                    feature_value = value_cell.get_text().strip()
                    
                    # Map Royal Queen Seeds fields to our schema
                    field_map = {
                        'Variety': 'variety',
                        'THC': 'thc',
                        'CBD': 'cbd',
                    ...
                    if feature_name in field_map and feature_value:
                        data[field_map[feature_name]] = feature_value
        
        return data

    def method2_description_mining(self, soup, url):
        """Method 2: Mine Royal Queen Seeds product descriptions"""
        data = {}
        ...
        if about_parts:
            ...
            patterns = {
                'genetics': r'(?:genetics|lineage|cross|bred from)[:\s]*([^.]+?)(?:\.|$)',
    ...
    def method3_advanced_patterns(self, soup, url):
        """Method 3: Royal Queen Seeds specific patterns"""
        data = {}
        
        # Extract strain name from product title
        title_selectors = ['h1.product-title', 'h1', '.product-name h1', '.page-title']
        for selector in title_selectors:
            title_elem = soup.select_one(selector)
            if title_elem:
                strain_name = title_elem.get_text().strip()
                # Clean Royal Queen Seeds naming patterns
                strain_name = re.sub(r'\s+', ' ', strain_name)
                strain_name = re.sub(r'\s*(Feminized|Auto|Autoflower|Seeds?)\s*', ' ', strain_name, re.IGNORECASE)
                data['strain_name'] = strain_name.strip()
                break
        
        # Royal Queen Seeds is always both seed bank and breeder
        data['breeder_name'] = 'Royal Queen Seeds'
        ...
        elif 'feminized' in url_lower:
            data['seed_type'] = 'Feminized'
            data['growth_type'] = 'Photoperiod'
        elif 'cbd' in url_lower:
            data['seed_type'] = 'Feminized'
            data['growth_type'] = 'Photoperiod'
        ...
        # Check availability
        if soup.find(text=re.compile(r'out of stock|sold out|unavailable', re.IGNORECASE)):
    ...
    def method4_fallback_extraction(self, soup, url):
        """Method 4: Universal fallback for Royal Queen Seeds"""
        data = {}
        ...
        if 'strain_name' not in data:
            ...
            for part in reversed(path_parts):
                if part and '.html' in part:
                    strain_name = part.replace('.html', '').replace('-', ' ').title()
                    # Clean common suffixes
                    strain_name = re.sub(r'\s+(Auto|Feminized|Seeds?)$', '', strain_name, re.IGNORECASE)
                    data['strain_name'] = strain_name.strip()
                    break
        ...
        if title and not data.get('strain_name'):
            ...
            if title_parts:
                ...
                potential_strain = re.sub(r'\s+(Auto|Feminized|Seeds?)$', '', potential_strain, re.IGNORECASE)
                data['strain_name'] = potential_strain
        
        return data

    def apply_4_methods(self, html_content, url):
        """Apply all 4 extraction methods with Royal Queen Seeds hardcoded values"""
        soup = BeautifulSoup(html_content, 'html.parser')
        
        strain_data = {
            'seed_bank': 'Royal Queen Seeds',
            'breeder_name': 'Royal Queen Seeds',  # Always the same
            'source_url': url,
        ...
        strain_data['strain_id'] = self.create_strain_id(strain_name, 'Royal Queen Seeds')
    ...
    def calculate_quality_score(self, strain_data):
        """Calculate weighted quality score optimized for Royal Queen Seeds"""
        field_weights = {
            # Core fields (required)
            'strain_name': 10,
            'breeder_name': 10,
            
            # Royal Queen Seeds strengths
            'thc': 9,                    # THC percentages
            'cbd': 8,                    # CBD content
            'yield_indoor': 8,           # Indoor yield data
            'yield_outdoor': 8,          # Outdoor yield data
            'flowering_time': 9,         # Flowering time
            'height_indoor': 7,          # Indoor height
            'height_outdoor': 7,         # Outdoor height
            'effects': 7,                # Effect descriptions
            'flavour': 6,                # Flavor profiles
            
            # Seed type classification
            'seed_type': 6,              # Feminized/Regular
            'growth_type': 6,            # Autoflower/Photoperiod
            'variety': 5,                # Variety classification
            
            # Genetics and breeding
            'genetic_background': 7,     # Genetic lineage
            'genetics_type': 6,          # Indica/Sativa ratios
            
            # Environmental data
            'climate': 5,                # Climate requirements
            'harvest_month': 5,          # Harvest timing
            
            # Additional info
            'about_info': 6,             # Product descriptions
    ...
    def create_strain_id(self, strain_name, breeder_name):
        combined = f"{strain_name}-{breeder_name}-rqs".lower()
        combined = re.sub(r'[^a-z0-9-]', '', combined.replace(' ', '-'))
        return combined[:50]

    def collect_strain_urls(self):
        """Phase 1: Collect all strain URLs from Royal Queen Seeds categories"""
        print("PHASE 1: Collecting Royal Queen Seeds strain URLs...")
        ...
        category_urls = [
            ...
            "https://www.royalqueenseeds.com/us/36-cbd-seeds"
        ]
        ...
        for category_url in category_urls:
            ...
            for link in soup.find_all('a', href=True):
                href = link.get('href')
                if href and ('/feminized-cannabis-seeds/' in href or 
                           '/autoflowering-cannabis-seeds/' in href or 
                           '/cbd-seeds/' in href):
                    if href.startswith('/'):
                        full_url = f"https://www.royalqueenseeds.com{href}"
                    else:
                        full_url = href
            ...
            if html:
                ...
                if strain_data['data_completeness_score'] >= 20:
                    try:
                        ...
                        print(f"  SUCCESS: {strain_data.get('strain_name', 'Unknown')} - Royal Queen Seeds")
                        ...
                        # Show key Royal Queen Seeds fields if present
                        if strain_data.get('thc'):
                            print(f"     THC: {strain_data['thc']}")
    ...
    def print_final_stats(self):
        ...
        print(f"\nROYAL QUEEN SEEDS ENHANCED SCRAPING COMPLETE!")
        ...
        print(f"Unique Features: Comprehensive cultivation data, dual yield measurements")

def main():
    scraper = RoyalQueenEnhanced4MethodScraper()
...
if __name__ == "__main__":
    print("ROYAL QUEEN SEEDS - ENHANCED 4-METHOD SCRAPER")
    print("Target: 100-200 strains with 95%+ success rate")
    print("Methods: Structured + Description + Patterns + Fallback")
    print("Hardcoded: seed_bank and breeder_name as 'Royal Queen Seeds'")
    print("Specializing in comprehensive cultivation data extraction")
    print("\n" + "="*60)
