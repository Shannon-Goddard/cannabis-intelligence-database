...
def extract_product_urls(html_content):
    ...
    for link in soup.find_all('a', href=True):
        href = link.get('href')
        if href and '/product/' in href and 'strain' in href:
            if not href.startswith('http'):
                href = 'https://neptuneseedbank.com' + href
            urls.append(href)
...
# Get strain URLs from product tags
tag_urls = [
...
all_urls = []
print("Finding strain URLs...")

for tag_url in tag_urls:
    print(f"Checking: {tag_url}")
    html = brightdata_request(tag_url, credentials)
    if html:
...
print(f"Total unique strains: {len(all_urls)}")

# Scrape each strain
successful = 0
for i, url in enumerate(all_urls[:50], 1):  # Limit to 50 for now
    print(f"[{i}/{min(50, len(all_urls))}] {url}")
...
print(f"\nComplete: {successful}/{min(50, len(all_urls))} strains saved")